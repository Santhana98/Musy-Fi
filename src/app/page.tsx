...
export default function Home() {
  ...
  const [videoInfoResult, setVideoInfoResult] = useState<string | null>(null);
  const [downloadResult, setDownloadResult] = useState<string | null>(null);

  const runTestYtDlp = async () => {
    setTestingYtDlp(true);
    setYtDlpVersion(null);
    try {
      const YtDlp = (window as any).Capacitor?.Plugins?.YtDlp;
      if (!YtDlp) {
        setYtDlpVersion('Capacitor YtDlp Plugin not available (not running on native Android device/emulator)');
        return;
      }
      const res = await YtDlp.testYtDlp();
      if (res && res.version) {
        setYtDlpVersion(`yt-dlp version: ${res.version}`);
      } else {
        setYtDlpVersion(`yt-dlp response empty: ${JSON.stringify(res)}`);
      }
    } catch (err: any) {
      setYtDlpVersion(`Error testing yt-dlp: ${err?.message || err}`);
    } finally {
      setTestingYtDlp(false);
    }
  };
  const runVideoInfoTest = async () => {
  try {
    setVideoInfoResult('Loading...');

    const YtDlp = (window as any).Capacitor?.Plugins?.YtDlp;

    if (!YtDlp) {
      setVideoInfoResult('Plugin not available');
      return;
    }

    const result = await YtDlp.getVideoInfo({
      url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'
    });

    setVideoInfoResult(JSON.stringify(result, null, 2));
  ...
  try {
    ...
    if (!YtDlp) {
      setDownloadResult('Plugin not available');
      return;
    }

    const result = await YtDlp.downloadSong({
      url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
      videoId: 'dQw4w9WgXcQ'
    });
...
if (result?.url) {
  try {
    ...
    console.log('[TEST] WebView URL:', webviewUrl);

    const response = await fetch(webviewUrl);
    const blob = await response.blob();
    const blobUrl = URL.createObjectURL(blob);
    console.log('[TEST] Blob Object URL:', blobUrl);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', paddingBottom: 160 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        {/* PoC Card for local yt-dlp execution */}
        <div style={{
          ...
          <h3 style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
            🤖 yt-dlp ARM64 Android PoC
          </h3>
          <p style={{ color: '#aaa', fontSize: 12, marginBottom: 12, lineHeight: '1.4' }}>
            Verify local execution of python3 yt-dlp using the embedded python runtime wrapper on ARM64 Android.
          </p>
          <button 
            onClick={runTestYtDlp}
            disabled={testingYtDlp}
            style={{
              background: testingYtDlp ? '#444' : 'linear-gradient(135deg, #e53935, #b71c1c)',
              color: '#fff',
              border: 'none',
              borderRadius: 8,
              padding: '10px 16px',
              fontSize: 13,
              fontWeight: 700,
              cursor: testingYtDlp ? 'not-allowed' : 'pointer',
              transition: 'all 0.2s',
            }}
          >
            {testingYtDlp ? 'Running...' : 'Run python3 yt-dlp --version'}
          </button>
          <div style={{ marginTop: 10 }}>
  <button
    onClick={runVideoInfoTest}
    style={{
      background: '#1976d2',
      color: '#fff',
      border: 'none',
    ...
    Test Audio Download
          ...
          {ytDlpVersion && (
            <div style={{
              ...
              color: ytDlpVersion.startsWith('Error') || ytDlpVersion.includes('not available') ? '#ff5252' : '#4caf50',
